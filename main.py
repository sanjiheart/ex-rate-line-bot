...
bank_options = {"004": "台灣銀行", "017": "兆豐商銀",
                "812": "台新銀行", "012": "台北富邦", "807": "永豐銀行"}
...
def hello():
    return "ex-rate-line-bot works fine!:-P"
...
def handle_text_message(event):
    if not handle_swear_word(event.source.user_id, event.message.text):
        if event.source.user_id not in cached_user_prefs:
            preference = user_prefs.get(event.source.user_id)
            # if None set default, else get from database
            if preference is None:
                cached_user_prefs[event.source.user_id] = {
                    "bank": "台新銀行", "ccy": "JPY"}
                user_prefs.create(event.source.user_id, "台新銀行", "JPY")
            else:
                cached_user_prefs[event.source.user_id] = {
                    "bank": preference["bank"], "ccy": preference["ccy"]}
        ...
        if user_input[0] not in user_options:
            ex_rate, last_updated = get_ex_rate(
                cached_user_prefs[event.source.user_id].get("bank"),
                cached_user_prefs[event.source.user_id].get("ccy"))
            reply_message = "### {bank_name} {ccy_abbre} ###\n即期匯率：{ex_rate}\n最後更新：{last_updated}".format(
                bank_name=cached_user_prefs[event.source.user_id].get(
                    "bank"),
                ccy_abbre=cached_user_prefs[event.source.user_id].get(
                    "ccy"),
                ex_rate=ex_rate,
            ...
            if user_input[0] == "bank":
                cached_user_prefs[event.source.user_id]["bank"] = bank_options.get(
                    user_input[1], "台新銀行")
                user_prefs.update(
                    event.source.user_id,
                    cached_user_prefs[event.source.user_id]["bank"],
                    cached_user_prefs[event.source.user_id]["ccy"])
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(text="預設銀行已變更為{}".format(cached_user_prefs[event.source.user_id]["bank"])))
            elif user_input[0] == "ccy":
                cached_user_prefs[event.source.user_id]["ccy"] = user_input[1] if user_input[1] in ccy_options else "JPY"
                user_prefs.update(
                    event.source.user_id,
                    cached_user_prefs[event.source.user_id]["bank"],
                    cached_user_prefs[event.source.user_id]["ccy"])
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(text="預設外幣已變更為{}".format(cached_user_prefs[event.source.user_id]["ccy"])))
            else:
                bank_options_repr = ""
                for code in sorted(bank_options):
                    bank_options_repr += "{name} {code}\n".format(
                        name=bank_options[code], code=code)
                ccy_options_repr = ""
                for abbre in sorted(ccy_options):
                    ccy_options_repr += "{name} {abbre}\n".format(
                        name=ccy_options[abbre], abbre=abbre)
...
def get_ex_rate(bank_name, ccy_abbre):
    api_uri = "https://tw.rter.info/json.php?t=currency&q=check&iso={ccy_abbre}".format(
        ccy_abbre=ccy_abbre)
